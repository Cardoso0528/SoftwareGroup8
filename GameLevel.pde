...
class LoseLevel extends GameLevel
{
  ...
  LoseLevel(PApplet applet)
  {
    super(applet);
       ...
       float accuracyReturn(){
    float accuracy = 0.0;
    if (totalLaunch > 0){
      accuracy = ((float)(totalHits)/(float)totalLaunch)*100;
    }
    if(accuracy >= 100){
       accuracy = 100; 
    }
  ...
  void start()
  {
    ...
    soundPlayer.stopBackgroundPlayer();
  ...
  {
    gameOver.update();
    if (!soundPlayer.loseGame.isPlaying()) {
...
class WinLevel extends GameLevel
{
  ...
  void update()
  {
    ...
    if (soundPlayer.winGame.isPlaying()) {
      ohYea.update();
